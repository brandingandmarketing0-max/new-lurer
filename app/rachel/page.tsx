...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const sessionId = `rachel_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const sessionKey = `rachel_visit_tracked_${sessionId}`;
    ...
    const send = () => {
      try {
        ...
        if (navigator.sendBeacon) {
          ...
          console.log("✅ Rachel Analytics - Page visit tracked via sendBeacon");
          
          // Mark this specific session as tracked
          localStorage.setItem(sessionKey, 'true');
          localStorage.setItem('rachel_last_tracked', new Date().toISOString());
          setHasTracked(true);
        } else {
          fetch("/api/track", {
            method: "POST",
            ...
            console.log("✅ Rachel Analytics - Page visit tracked via fetch");
            ...
            console.error("❌ Rachel Analytics - Page visit tracking failed:", error);
          });
        }
      } catch (error) {
        console.error("❌ Failed to track Rachel analytics:", error);
      }
    };
  ...
  const handleConfirmAge = () => {
    setShowAgeWarning(false);
    const chars = [104,116,116,112,115,58,47,47,111,110,108,121,102,97,110,115,46,99,111,109,47,121,111,117,114,103,105,114,108,114,120,99,104];
