...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const sessionId = `libby_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const sessionKey = `libby_visit_tracked_${sessionId}`;
    ...
    const send = () => {
      try {
        ...
        if (navigator.sendBeacon) {
          ...
          console.log("✅ Libby Analytics - Page visit tracked via sendBeacon");
          
          // Mark this specific session as tracked
          localStorage.setItem(sessionKey, 'true');
          localStorage.setItem('libby_last_tracked', new Date().toISOString());
          setHasTracked(true);
        } else {
          fetch("/api/track", {
            method: "POST",
            ...
            console.log("✅ Libby Analytics - Page visit tracked via fetch");
            ...
            console.error("❌ Libby Analytics - Page visit tracking failed:", error);
          });
        }
      } catch (error) {
        console.error("❌ Failed to track Libby analytics:", error);
      }
    };
  ...
  const handleConfirmAge = () => {
    setShowAgeWarning(false);
    const chars = [104,116,116,112,115,58,47,47,111,110,108,121,102,97,110,115,46,99,111,109,47,108,105,98,100,97,119,115,111,110];
              ...
              <div className="flex flex-col items-center space-y-6">
                ...
                <div className="relative group">
                  ...
                  <Avatar className="relative h-28 w-28 border-4 border-[#B6997B]/20 shadow-lg">
                    <AvatarImage src="https://2eovi9l2gc.ufs.sh/f/XQC8QM7wDFrtevA1PkKDVTbWye4LIz7jlNHsC1uoncERaqJ3" alt="Summermae" className="object-cover" />
          ...
          <div onClick={handleExclusiveContentClick}>
            <Card className="mt-6 relative overflow-hidden border border-[#B6997B]/50 bg-[#B6997B]/10 shadow-lg backdrop-blur-sm cursor-pointer hover:shadow-xl transition-shadow duration-300">
                <CardContent className="p-0">
                  <div className="relative group">
                    <Image
                      src="https://2eovi9l2gc.ufs.sh/f/XQC8QM7wDFrtKoDzSTMoIFQf5TiLHjV7u0D63yYgCwEtxB8Z"
