...
interface PrettygreeneyesxxAnalyticsData {
  id: number;
  page: string;
  referrer: string;
...
export default function PrettygreeneyesxxAnalyticsPage() {
  return (
    <ProtectedRoute>
      <PrettygreeneyesxxAnalyticsContent />
    </ProtectedRoute>
  );
}

function PrettygreeneyesxxAnalyticsContent() {
  const [analyticsData, setAnalyticsData] = useState<PrettygreeneyesxxAnalyticsData[]>([]);
  ...
  const fetchAnalytics = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/track?page=prettygreeneyesxx');
      if (!response.ok) {
      ...
      <div className="min-h-screen bg-white p-4">
        <div className="max-w-6xl mx-auto">
          <div className="text-gray-800 text-center py-20">
            ...
            <h1 className="text-2xl font-bold">Loading prettygreeneyesxx Analytics...</h1>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-white p-4">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Analytics for prettygreeneyesxx</h1>
        ...
        <Card className="mb-6 border-[#B19272]">
          ...
          <CardContent>
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[#B19272] font-medium">viewit.bio/prettygreeneyesxx</span>
            </div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <div className="space-y-4">
                {topReferrers.map(([referrer, count], index) => (
                  <div key={referrer} className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="h-4 w-4 rounded" style={{ backgroundColor: index === 0 ? '#B19272' : '#E5E7EB' }} />
                      <span className="text-gray-700 font-medium">{referrer}</span>
                    </div>
                    <div className="flex items-center gap-4">
                      <div className="w-32 bg-gray-200 rounded-full h-2">
                        <div className="h-2 rounded-full" style={{ width: `${(count / Math.max(...topReferrers.map(([,c]) => c))) * 100}%`, backgroundColor: index === 0 ? '#B19272' : '#9CA3AF' }} />
                      </div>
