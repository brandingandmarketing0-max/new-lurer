...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const sessionId = `aimee_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const sessionKey = `aimee_visit_tracked_${sessionId}`;
    ...
    const send = () => {
      try {
        ...
        if (navigator.sendBeacon) {
          ...
          console.log("✅ Aimee Analytics - Page visit tracked via sendBeacon");
          
          // Mark this specific session as tracked
          localStorage.setItem(sessionKey, 'true');
          localStorage.setItem('aimee_last_tracked', new Date().toISOString());
          setHasTracked(true);
        } else {
          fetch("/api/track", {
            method: "POST",
            ...
            console.log("✅ Aimee Analytics - Page visit tracked via fetch");
            ...
            console.error("❌ Aimee Analytics - Page visit tracking failed:", error);
          });
        }
      } catch (error) {
        console.error("❌ Failed to track Aimee analytics:", error);
      }
    };
      ...
      <div className="flex min-h-screen items-center justify-center px-2">
        <div className="w-full max-w-md mx-auto">
          ...
          <div onClick={handleExclusiveContentClick}>
            <Link href="https://onlyfans.com/petiteblondee" target="_blank" rel="noopener noreferrer">
