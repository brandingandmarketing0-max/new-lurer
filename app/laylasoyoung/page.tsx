...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const sessionId = `laylasoyoung_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const sessionKey = `laylasoyoung_visit_tracked_${sessionId}`;
    ...
    const send = () => {
      try {
        ...
        if (navigator.sendBeacon) {
          ...
          console.log("✅ Laylasoyoung Analytics - Page visit tracked via sendBeacon");
          
          // Mark this specific session as tracked
          localStorage.setItem(sessionKey, 'true');
          localStorage.setItem('laylasoyoung_last_tracked', new Date().toISOString());
          setHasTracked(true);
        } else {
          fetch("/api/track", {
            method: "POST",
            ...
            console.log("✅ Laylasoyoung Analytics - Page visit tracked via fetch");
            ...
            console.error("❌ Laylasoyoung Analytics - Page visit tracking failed:", error);
          });
        }
      } catch (error) {
        console.error("❌ Failed to track Laylasoyoung analytics:", error);
      }
    };
  ...
  const handleConfirmAge = () => {
    setShowAgeWarning(false);
    const chars = [104,116,116,112,115,58,47,47,111,110,108,121,102,97,110,115,46,99,111,109,47,108,97,121,108,97,115,111,121,111,117,110,103];
