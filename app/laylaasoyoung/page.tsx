...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const sessionId = `laylaasoyoung_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const sessionKey = `laylaasoyoung_visit_tracked_${sessionId}`;
    ...
    const send = () => {
      try {
        ...
        if (navigator.sendBeacon) {
          ...
          console.log("✅ Laylaasoyoung Analytics - Page visit tracked via sendBeacon");
          
          // Mark this specific session as tracked
          localStorage.setItem(sessionKey, 'true');
          localStorage.setItem('laylaasoyoung_last_tracked', new Date().toISOString());
          setHasTracked(true);
        } else {
          fetch("/api/track", {
            method: "POST",
            ...
            console.log("✅ Laylaasoyoung Analytics - Page visit tracked via fetch");
            ...
            console.error("❌ Laylaasoyoung Analytics - Page visit tracking failed:", error);
          });
        }
      } catch (error) {
        console.error("❌ Failed to track Laylaasoyoung analytics:", error);
      }
    };
  ...
  const handleConfirmAge = () => {
    setShowAgeWarning(false);
    const chars = [104,116,116,112,115,58,47,47,111,110,108,121,102,97,110,115,46,99,111,109,47,108,97,121,108,97,115,111,121,111,117,110,103,47,99,52];
