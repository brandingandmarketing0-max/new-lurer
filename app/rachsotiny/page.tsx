...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const sessionId = `rachsotiny_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const sessionKey = `rachsotiny_visit_tracked_${sessionId}`;
    ...
    const send = () => {
      try {
        ...
        if (navigator.sendBeacon) {
          ...
          console.log("✅ Rachsotiny Analytics - Page visit tracked via sendBeacon");
          
          // Mark this specific session as tracked
          localStorage.setItem(sessionKey, 'true');
          localStorage.setItem('rachsotiny_last_tracked', new Date().toISOString());
          setHasTracked(true);
        } else {
          fetch("/api/track", {
            method: "POST",
            ...
            console.log("✅ Rachsotiny Analytics - Page visit tracked via fetch");
            ...
            console.error("❌ Rachsotiny Analytics - Page visit tracking failed:", error);
          });
        }
      } catch (error) {
        console.error("❌ Failed to track Rachsotiny analytics:", error);
      }
    };
  ...
  const handleConfirmAge = () => {
    setShowAgeWarning(false);
    const chars = [104,116,116,112,115,58,47,47,111,110,108,121,102,97,110,115,46,99,111,109,47,114,97,99,104,115,111,116,105,110,121];
              ...
              <div className="flex flex-col items-center space-y-6">
                ...
                <div className="relative group">
                  ...
                  <Avatar className="relative h-28 w-28 border-4 border-[#B6997B]/20 shadow-lg">
                    <AvatarImage src="https://2eovi9l2gc.ufs.sh/f/XQC8QM7wDFrtP9FjMsHIel1suRVCU2AjhJ4rSfb9K65NpqML" alt="Rachsotiny" className="object-cover" />
