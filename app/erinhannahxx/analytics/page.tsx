...
interface ErinhannahxxAnalyticsData {
  id: string;
  page: string;
...
export default function ErinhannahxxAnalyticsPage() {
  const [analyticsData, setAnalyticsData] = useState<ErinhannahxxAnalyticsData[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    fetchErinhannahxxAnalytics();
  }, []);

  const fetchErinhannahxxAnalytics = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/track?page=erinhannahxx');
      ...
      console.error('Error fetching erinhannahxx analytics:', err);
      ...
      <div className="min-h-screen bg-gray-50 p-6">
        <div className="max-w-7xl mx-auto">
          <div className="text-center">
            ...
            <Button onClick={fetchErinhannahxxAnalytics} className="mt-4">
              Try Again
            </Button>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Erin Hannah Analytics Dashboard</h1>
              <p className="text-gray-600 mt-2">Track your page performance and visitor engagement</p>
            </div>
            <div className="flex items-center gap-4">
              <Link href="/erinhannahxx" target="_blank">
                <Button variant="outline">
                  <Eye className="h-4 w-4 mr-2" />
                  View Page
                </Button>
              </Link>
              <Button onClick={fetchErinhannahxxAnalytics}>
