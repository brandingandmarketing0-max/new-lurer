...
export async function GET() {
  try {
    //console.log("[MEGANN API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('megann_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      //console.error("[MEGANN API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    //console.log("[MEGANN API] Total records in database:", count);
    ...
    for (let page = 0; page < totalPages; page++) {
      ...
      const { data: chunk, error: chunkError } = await supabase
        ...
        .range(start, end);
      if (chunkError) {
        //console.error("[MEGANN API] Supabase fetch error (page", page, "):", chunkError);
    ...
    //console.log("[MEGANN API] Successfully fetched data:", allRows.length, "records");
    ...
    //console.error("[MEGANN API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    //console.log(`[MEGANN API] Received analytics request for page: ${page}`);
    //console.log(`[MEGANN API] Raw referrer: ${referrer}`);
    //console.log(`[MEGANN API] Click type: ${click_type || 'page_visit'}`);
    ...
    //console.log(`[MEGANN API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      //console.error("[MEGANN API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[MEGANN API] Successfully saved to Supabase:", data);
    ...
    //console.error("[MEGANN API] Error storing analytics:", error);
