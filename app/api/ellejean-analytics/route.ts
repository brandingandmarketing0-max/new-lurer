...
export async function GET() {
  try {
    //console.log("[ELLEJEAN API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('ellejean_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      //console.error("[ELLEJEAN API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    //console.log("[ELLEJEAN API] Total records in database:", count);
    ...
    if (error) {
      //console.error("[ELLEJEAN API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[ELLEJEAN API] Successfully fetched data:", data?.length, "records");
    ...
    //console.error("[ELLEJEAN API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    //console.log(`[ELLEJEAN API] Received analytics request for page: ${page}`);
    //console.log(`[ELLEJEAN API] Raw referrer: ${referrer}`);
    //console.log(`[ELLEJEAN API] Click type: ${click_type || 'page_visit'}`);
    ...
    //console.log(`[ELLEJEAN API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      //console.error("[ELLEJEAN API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[ELLEJEAN API] Successfully saved to Supabase:", data);
    ...
    //console.error("[ELLEJEAN API] Error storing analytics:", error);
