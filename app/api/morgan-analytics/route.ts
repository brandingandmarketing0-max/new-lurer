...
export async function GET() {
  try {
    console.log("[MORGAN API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('morgan_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      console.error("[MORGAN API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    console.log("[MORGAN API] Total records in database:", count);
    ...
    if (error) {
      console.error("[MORGAN API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[MORGAN API] Successfully fetched data:", data?.length, "records");
    ...
    console.error("[MORGAN API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`[MORGAN API] Received analytics request for page: ${page}`);
    console.log(`[MORGAN API] Raw referrer: ${referrer}`);
    console.log(`[MORGAN API] Click type: ${click_type || 'page_visit'}`);
    ...
    console.log(`[MORGAN API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      console.error("[MORGAN API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[MORGAN API] Successfully saved to Supabase:", data);
    ...
    console.error("[MORGAN API] Error storing analytics:", error);
