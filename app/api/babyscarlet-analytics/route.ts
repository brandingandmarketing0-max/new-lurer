...
export async function GET() {
  try {
    //console.log("[BABYSCARLET API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('babyscarlet_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      //console.error("[BABYSCARLET API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    //console.log("[BABYSCARLET API] Total records in database:", count);
    ...
    if (error) {
      //console.error("[BABYSCARLET API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[BABYSCARLET API] Successfully fetched data:", data?.length, "records");
    ...
    //console.error("[BABYSCARLET API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    //console.log(`[BABYSCARLET API] Received analytics request for page: ${page}`);
    //console.log(`[BABYSCARLET API] Raw referrer: ${referrer}`);
    //console.log(`[BABYSCARLET API] Click type: ${click_type || 'page_visit'}`);
    ...
    //console.log(`[BABYSCARLET API] Processed referrer: ${readableReferrer}`);

    const analyticsData: BrookeAnalyticsData = {
      page: page || "babyscarlet",
      referrer: referrer || "",
      readable_referrer: readableReferrer,
      user_agent: userAgent,
      ip_address: ip,
      timestamp: timestamp || new Date().toISOString(),
      pathname: pathname || "/babyscarlet",
    ...
    if (error) {
      //console.error("[BABYSCARLET API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[BABYSCARLET API] Successfully saved to Supabase:", data);
    ...
    //console.error("[BABYSCARLET API] Error storing analytics:", error);
