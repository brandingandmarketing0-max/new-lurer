...
export async function GET() {
  try {
    console.log("[CHLOEELIZABETH API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('chloeelizabeth_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      console.error("[CHLOEELIZABETH API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    console.log("[CHLOEELIZABETH API] Total records in database:", count);
    ...
    if (error) {
      console.error("[CHLOEELIZABETH API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[CHLOEELIZABETH API] Successfully fetched data:", data?.length, "records");
    ...
    console.error("[CHLOEELIZABETH API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`[CHLOEELIZABETH API] Received analytics request for page: ${page}`);
    console.log(`[CHLOEELIZABETH API] Raw referrer: ${referrer}`);
    console.log(`[CHLOEELIZABETH API] Click type: ${click_type || 'page_visit'}`);
    ...
    console.log(`[CHLOEELIZABETH API] Processed referrer: ${readableReferrer}`);

    const analyticsData: BrookeAnalyticsData = {
      page: page || "chloeelizabeth",
      referrer: referrer || "",
      readable_referrer: readableReferrer,
      user_agent: userAgent,
      ip_address: ip,
      timestamp: timestamp || new Date().toISOString(),
      pathname: pathname || "/chloeelizabeth",
    ...
    if (error) {
      console.error("[CHLOEELIZABETH API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[CHLOEELIZABETH API] Successfully saved to Supabase:", data);
    ...
    console.error("[CHLOEELIZABETH API] Error storing analytics:", error);
