...
export async function GET() {
  try {
    console.log("[EM API] Fetching analytics data from Supabase");
    ...
    if (countError) {
      console.error("[EM API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    console.log("[EM API] Total records in database:", count);
    ...
    if (error) {
      console.error("[EM API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[EM API] Successfully fetched data:", data?.length, "records");
    ...
    console.error("[EM API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`[EM API] Received analytics request for page: ${page}`);
    console.log(`[EM API] Raw referrer: ${referrer}`);
    console.log(`[EM API] Click type: ${click_type || 'page_visit'}`);
    ...
    console.log(`[EM API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      console.error("[EM API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[EM API] Successfully saved to Supabase:", data);
    ...
    console.error("[EM API] Error storing analytics:", error);
