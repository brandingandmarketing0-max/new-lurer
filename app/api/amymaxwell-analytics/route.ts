...
export async function GET() {
  try {
    console.log("[AMYMAXWELL API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('amymaxwell_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      console.error("[AMYMAXWELL API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    console.log("[AMYMAXWELL API] Total records in database:", count);
    ...
    if (error) {
      console.error("[AMYMAXWELL API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[AMYMAXWELL API] Successfully fetched data:", data?.length, "records");
    ...
    console.error("[AMYMAXWELL API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`[AMYMAXWELL API] Received analytics request for page: ${page}`);
    console.log(`[AMYMAXWELL API] Raw referrer: ${referrer}`);
    console.log(`[AMYMAXWELL API] Click type: ${click_type || 'page_visit'}`);
    ...
    console.log(`[AMYMAXWELL API] Processed referrer: ${readableReferrer}`);

    const analyticsData: BrookeAnalyticsData = {
      page: page || "amymaxwell",
      referrer: referrer || "",
      readable_referrer: readableReferrer,
      user_agent: userAgent,
      ip_address: ip,
      timestamp: timestamp || new Date().toISOString(),
      pathname: pathname || "/amymaxwell",
    ...
    if (error) {
      console.error("[AMYMAXWELL API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[AMYMAXWELL API] Successfully saved to Supabase:", data);
    ...
    console.error("[AMYMAXWELL API] Error storing analytics:", error);
