...
export async function GET() {
  try {
    //console.log("[ABBIEHALL API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('abbiehall_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      //console.error("[ABBIEHALL API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    //console.log("[ABBIEHALL API] Total records in database:", count);
    ...
    if (error) {
      //console.error("[ABBIEHALL API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[ABBIEHALL API] Successfully fetched data:", data?.length, "records");
    ...
    //console.error("[ABBIEHALL API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    //console.log(`[ABBIEHALL API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      //console.error("[ABBIEHALL API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[ABBIEHALL API] Successfully saved to Supabase:", data);
    ...
    //console.error("[ABBIEHALL API] Error storing analytics:", error);
