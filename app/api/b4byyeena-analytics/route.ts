...
export async function GET() {
  try {
    //console.log("[B4BYYEENA API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('b4byyeena_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      //console.error("[B4BYYEENA API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    //console.log("[B4BYYEENA API] Total records in database:", count);
    ...
    if (error) {
      //console.error("[B4BYYEENA API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[B4BYYEENA API] Successfully fetched data:", data?.length, "records");
    ...
    //console.error("[B4BYYEENA API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    //console.log(`[B4BYYEENA API] Received analytics request for page: ${page}`);
    //console.log(`[B4BYYEENA API] Raw referrer: ${referrer}`);
    //console.log(`[B4BYYEENA API] Click type: ${click_type || 'page_visit'}`);
    ...
    //console.log(`[B4BYYEENA API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      //console.error("[B4BYYEENA API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[B4BYYEENA API] Successfully saved to Supabase:", data);
    ...
    //console.error("[B4BYYEENA API] Error storing analytics:", error);
