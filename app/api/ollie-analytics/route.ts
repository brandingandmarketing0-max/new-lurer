...
export async function GET() {
  try {
    //console.log("[OLLIE API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('ollie_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      //console.error("[OLLIE API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    //console.log("[OLLIE API] Total records in database:", count);
    ...
    if (error) {
      //console.error("[OLLIE API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[OLLIE API] Successfully fetched data:", data?.length, "records");
    ...
    //console.error("[OLLIE API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    //console.log(`[OLLIE API] Received analytics request for page: ${page}`);
    //console.log(`[OLLIE API] Raw referrer: ${referrer}`);
    //console.log(`[OLLIE API] Click type: ${click_type || 'page_visit'}`);
    ...
    //console.log(`[OLLIE API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      //console.error("[OLLIE API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[OLLIE API] Successfully saved to Supabase:", data);
    ...
    //console.error("[OLLIE API] Error storing analytics:", error);
