...
export async function GET() {
  try {
    //console.log("[LAURDUNNE API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('laurdunne_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      //console.error("[LAURDUNNE API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    //console.log("[LAURDUNNE API] Total records in database:", count);
    ...
    if (error) {
      //console.error("[LAURDUNNE API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[LAURDUNNE API] Successfully fetched data:", data?.length, "records");
    ...
    //console.error("[LAURDUNNE API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    //console.log(`[LAURDUNNE API] Received analytics request for page: ${page}`);
    //console.log(`[LAURDUNNE API] Raw referrer: ${referrer}`);
    //console.log(`[LAURDUNNE API] Click type: ${click_type || 'page_visit'}`);
    ...
    //console.log(`[LAURDUNNE API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      //console.error("[LAURDUNNE API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[LAURDUNNE API] Successfully saved to Supabase:", data);
    ...
    //console.error("[LAURDUNNE API] Error storing analytics:", error);
