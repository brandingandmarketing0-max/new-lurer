...
export async function GET() {
  try {
    //console.log("[RACHEL API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('rachel_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      //console.error("[RACHEL API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    //console.log("[RACHEL API] Total records in database:", count);
    ...
    if (error) {
      //console.error("[RACHEL API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[RACHEL API] Successfully fetched data:", data?.length, "records");
    ...
    //console.error("[RACHEL API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    //console.log(`[RACHEL API] Received analytics request for page: ${page}`);
    //console.log(`[RACHEL API] Raw referrer: ${referrer}`);
    //console.log(`[RACHEL API] Click type: ${click_type || 'page_visit'}`);
    ...
    //console.log(`[RACHEL API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      //console.error("[RACHEL API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[RACHEL API] Successfully saved to Supabase:", data);
    ...
    //console.error("[RACHEL API] Error storing analytics:", error);
