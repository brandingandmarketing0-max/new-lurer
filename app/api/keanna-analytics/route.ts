...
export async function GET() {
  try {
    console.log("[KEANNA API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('keanna_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      console.error("[KEANNA API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    console.log("[KEANNA API] Total records in database:", count);
    ...
    if (error) {
      console.error("[KEANNA API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[KEANNA API] Successfully fetched data:", data?.length, "records");
    ...
    console.error("[KEANNA API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`[KEANNA API] Received analytics request for page: ${page}`);
    console.log(`[KEANNA API] Raw referrer: ${referrer}`);
    console.log(`[KEANNA API] Click type: ${click_type || 'page_visit'}`);
    ...
    console.log(`[KEANNA API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      console.error("[KEANNA API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[KEANNA API] Successfully saved to Supabase:", data);
    ...
    console.error("[KEANNA API] Error storing analytics:", error);
