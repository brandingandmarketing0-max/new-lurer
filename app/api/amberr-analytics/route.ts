...
export async function GET() {
  try {
    //console.log("[AMBERR API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('amberr_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      //console.error("[AMBERR API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    //console.log("[AMBERR API] Total records in database:", count);
    ...
    if (error) {
      //console.error("[AMBERR API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[AMBERR API] Successfully fetched data:", data?.length, "records");
    ...
    //console.error("[AMBERR API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    //console.log(`[AMBERR API] Received analytics request for page: ${page}`);
    //console.log(`[AMBERR API] Raw referrer: ${referrer}`);
    //console.log(`[AMBERR API] Click type: ${click_type || 'page_visit'}`);
    ...
    //console.log(`[AMBERR API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      //console.error("[AMBERR API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[AMBERR API] Successfully saved to Supabase:", data);
    ...
    //console.error("[AMBERR API] Error storing analytics:", error);
