...
export async function GET() {
  try {
    //console.log("[AMYLEIGH API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('amyleigh_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      //console.error("[AMYLEIGH API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    //console.log("[AMYLEIGH API] Total records in database:", count);
    ...
    if (error) {
      //console.error("[AMYLEIGH API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[AMYLEIGH API] Successfully fetched data:", data?.length, "records");
    ...
    //console.error("[AMYLEIGH API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    //console.log(`[AMYLEIGH API] Received analytics request for page: ${page}`);
    //console.log(`[AMYLEIGH API] Raw referrer: ${referrer}`);
    //console.log(`[AMYLEIGH API] Click type: ${click_type || 'page_visit'}`);
    ...
    //console.log(`[AMYLEIGH API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      //console.error("[AMYLEIGH API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[AMYLEIGH API] Successfully saved to Supabase:", data);
    ...
    //console.error("[AMYLEIGH API] Error storing analytics:", error);
