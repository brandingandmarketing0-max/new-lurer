...
export async function GET() {
  try {
    //console.log("[POPPY API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('poppy_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      //console.error("[POPPY API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    //console.log("[POPPY API] Total records in database:", count);
    ...
    if (error) {
      //console.error("[POPPY API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[POPPY API] Successfully fetched data:", data?.length, "records");
    ...
    //console.error("[POPPY API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    //console.log(`[POPPY API] Received analytics request for page: ${page}`);
    //console.log(`[POPPY API] Raw referrer: ${referrer}`);
    //console.log(`[POPPY API] Click type: ${click_type || 'page_visit'}`);
    ...
    //console.log(`[POPPY API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      //console.error("[POPPY API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[POPPY API] Successfully saved to Supabase:", data);
    ...
    //console.error("[POPPY API] Error storing analytics:", error);
