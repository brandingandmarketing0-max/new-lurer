...
export async function GET() {
  try {
    ...
    // If count is available, paginate deterministically; otherwise keep fetching until a short page
    ...
    return NextResponse.json({ 
      ...
      headers: {
        // Cache at the edge to reduce repeated compute when viewing dashboards
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Client-side dedupe: drop repeated page_visit from same IP+UA within 30 minutes
    const dedupeKey = `${ip}|${ua}|${click_type || 'page_visit'}|${pathname || ''}`;
    // In a serverless environment without shared cache, this is best-effort; still helps within a single container
    // Using header hints to reduce noise from prefetch/pre-render
