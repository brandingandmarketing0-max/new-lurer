...
export async function GET() {
  try {
    console.log("[BABYYEENA API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('babyyeena_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      console.error("[BABYYEENA API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    console.log("[BABYYEENA API] Total records in database:", count);
    ...
    if (error) {
      console.error("[BABYYEENA API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[BABYYEENA API] Successfully fetched data:", data?.length, "records");
    ...
    console.error("[BABYYEENA API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`[BABYYEENA API] Received analytics request for page: ${page}`);
    console.log(`[BABYYEENA API] Raw referrer: ${referrer}`);
    console.log(`[BABYYEENA API] Click type: ${click_type || 'page_visit'}`);
    ...
    console.log(`[BABYYEENA API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      console.error("[BABYYEENA API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[BABYYEENA API] Successfully saved to Supabase:", data);
    ...
    console.error("[BABYYEENA API] Error storing analytics:", error);
