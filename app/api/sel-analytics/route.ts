...
export async function GET() {
  try {
    //console.log("[SEL API] Fetching analytics data from Supabase");
    ...
    if (countError) {
      //console.error("[SEL API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    //console.log("[SEL API] Total records in database:", count);
    ...
    if (error) {
      //console.error("[SEL API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[SEL API] Successfully fetched data:", data?.length, "records");
    ...
    //console.error("[SEL API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    //console.log(`[SEL API] Received analytics request for page: ${page}`);
    //console.log(`[SEL API] Raw referrer: ${referrer}`);
    //console.log(`[SEL API] Click type: ${click_type || 'page_visit'}`);
    ...
    //console.log(`[SEL API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      //console.error("[SEL API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[SEL API] Successfully saved to Supabase:", data);
    ...
    //console.error("[SEL API] Error storing analytics:", error);
