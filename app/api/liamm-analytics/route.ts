...
export async function GET() {
  try {
    console.log("[LIAMM API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('liamm_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      console.error("[LIAMM API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    console.log("[LIAMM API] Total records in database:", count);
    ...
    if (error) {
      console.error("[LIAMM API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[LIAMM API] Successfully fetched data:", data?.length, "records");
    ...
    console.error("[LIAMM API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`[LIAMM API] Received analytics request for page: ${page}`);
    console.log(`[LIAMM API] Raw referrer: ${referrer}`);
    console.log(`[LIAMM API] Click type: ${click_type || 'page_visit'}`);
    ...
    console.log(`[LIAMM API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      console.error("[LIAMM API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[LIAMM API] Successfully saved to Supabase:", data);
    ...
    console.error("[LIAMM API] Error storing analytics:", error);
