...
export async function GET() {
  try {
    console.log("[CHLOETAMI API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('chloetami_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      console.error("[CHLOETAMI API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    console.log("[CHLOETAMI API] Total records in database:", count);
    ...
    if (error) {
      console.error("[CHLOETAMI API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[CHLOETAMI API] Successfully fetched data:", data?.length, "records");
    ...
    console.error("[CHLOETAMI API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`[CHLOETAMI API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      console.error("[CHLOETAMI API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[CHLOETAMI API] Successfully saved to Supabase:", data);
    ...
    console.error("[CHLOETAMI API] Error storing analytics:", error);
