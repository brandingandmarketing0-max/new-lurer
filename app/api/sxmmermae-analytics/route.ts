...
export async function GET() {
  try {
    console.log("[SXMMERMAE API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('sxmmermae_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      console.error("[SXMMERMAE API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    console.log("[SXMMERMAE API] Total records in database:", count);
    ...
    if (error) {
      console.error("[SXMMERMAE API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[SXMMERMAE API] Successfully fetched data:", data?.length, "records");
    ...
    console.error("[SXMMERMAE API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`[SXMMERMAE API] Received analytics request for page: ${page}`);
    console.log(`[SXMMERMAE API] Raw referrer: ${referrer}`);
    console.log(`[SXMMERMAE API] Click type: ${click_type || 'page_visit'}`);
    ...
    console.log(`[SXMMERMAE API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      console.error("[SXMMERMAE API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[SXMMERMAE API] Successfully saved to Supabase:", data);
    ...
    console.error("[SXMMERMAE API] Error storing analytics:", error);
