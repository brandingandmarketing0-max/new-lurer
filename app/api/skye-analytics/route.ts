...
export async function GET() {
  try {
    ...
    console.log("[SKYE API] Successfully fetched data:", data?.length, "records");
    ...
    console.error("[SKYE API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`[SKYE API] Received analytics request for page: ${page}`);
    console.log(`[SKYE API] Raw referrer: ${referrer}`);
    console.log(`[SKYE API] Click type: ${click_type || 'page_visit'}`);
    ...
    console.log(`[SKYE API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      console.error("[SKYE API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[SKYE API] Successfully saved to Supabase:", data);
    ...
    console.error("[SKYE API] Error storing analytics:", error);
