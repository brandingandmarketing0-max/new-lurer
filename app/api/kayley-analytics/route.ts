...
export async function GET() {
  try {
    console.log("[KAYLEY API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('kayley_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      console.error("[KAYLEY API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    console.log("[KAYLEY API] Total records in database:", count);
    ...
    if (error) {
      console.error("[KAYLEY API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[KAYLEY API] Successfully fetched data:", data?.length, "records");
    ...
    console.error("[KAYLEY API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`[KAYLEY API] Received analytics request for page: ${page}`);
    console.log(`[KAYLEY API] Raw referrer: ${referrer}`);
    console.log(`[KAYLEY API] Click type: ${click_type || 'page_visit'}`);
    ...
    console.log(`[KAYLEY API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      console.error("[KAYLEY API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[KAYLEY API] Successfully saved to Supabase:", data);
    ...
    console.error("[KAYLEY API] Error storing analytics:", error);
