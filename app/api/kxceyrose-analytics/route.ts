...
export async function GET() {
  try {
    console.log("[KXCEYROSE API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('kxceyrose_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      console.error("[KXCEYROSE API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    console.log("[KXCEYROSE API] Total records in database:", count);
    ...
    if (error) {
      console.error("[KXCEYROSE API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[KXCEYROSE API] Successfully fetched data:", data?.length, "records");
    ...
    console.error("[KXCEYROSE API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`[KXCEYROSE API] Received analytics request for page: ${page}`);
    console.log(`[KXCEYROSE API] Raw referrer: ${referrer}`);
    console.log(`[KXCEYROSE API] Click type: ${click_type || 'page_visit'}`);
    ...
    console.log(`[KXCEYROSE API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      console.error("[KXCEYROSE API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[KXCEYROSE API] Successfully saved to Supabase:", data);
    ...
    console.error("[KXCEYROSE API] Error storing analytics:", error);
