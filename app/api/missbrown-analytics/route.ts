...
export async function GET() {
  try {
    //console.log("[MISSBROWN API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('missbrown_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      //console.error("[MISSBROWN API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    //console.log("[MISSBROWN API] Total records in database:", count);
    ...
    if (error) {
      //console.error("[MISSBROWN API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[MISSBROWN API] Successfully fetched data:", data?.length, "records");
    ...
    //console.error("[MISSBROWN API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    //console.log(`[MISSBROWN API] Received analytics request for page: ${page}`);
    //console.log(`[MISSBROWN API] Raw referrer: ${referrer}`);
    //console.log(`[MISSBROWN API] Click type: ${click_type || 'page_visit'}`);
    ...
    //console.log(`[MISSBROWN API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      //console.error("[MISSBROWN API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[MISSBROWN API] Successfully saved to Supabase:", data);
    ...
    //console.error("[MISSBROWN API] Error storing analytics:", error);
