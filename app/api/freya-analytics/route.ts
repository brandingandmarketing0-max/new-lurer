...
export async function GET() {
  try {
    //console.log("[FREYA API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('freya_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      //console.error("[FREYA API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    //console.log("[FREYA API] Total records in database:", count);
    ...
    if (error) {
      //console.error("[FREYA API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[FREYA API] Successfully fetched data:", data?.length, "records");
    ...
    //console.error("[FREYA API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    //console.log(`[FREYA API] Received analytics request for page: ${page}`);
    //console.log(`[FREYA API] Raw referrer: ${referrer}`);
    //console.log(`[FREYA API] Click type: ${click_type || 'page_visit'}`);
    ...
    //console.log(`[FREYA API] Processed referrer: ${readableReferrer}`);
    ...
    if (error) {
      //console.error("[FREYA API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    //console.log("[FREYA API] Successfully saved to Supabase:", data);
    ...
    //console.error("[FREYA API] Error storing analytics:", error);
