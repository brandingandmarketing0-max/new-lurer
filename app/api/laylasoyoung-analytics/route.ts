...
export async function GET() {
  try {
    console.log("[LAYLASOYOUNG API] Fetching analytics data from Supabase");
    
    // First, get the total count
    const { count, error: countError } = await supabase
      .from('laylasoyoung_analytics')
      .select('*', { count: 'exact', head: true });

    if (countError) {
      console.error("[LAYLASOYOUNG API] Count error:", countError);
      return NextResponse.json({ success: false, error: countError.message }, { status: 500 });
    }

    console.log("[LAYLASOYOUNG API] Total records in database:", count);
    ...
    if (error) {
      console.error("[LAYLASOYOUNG API] Supabase fetch error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[LAYLASOYOUNG API] Successfully fetched data:", data?.length, "records");
    ...
    console.error("[LAYLASOYOUNG API] Error fetching analytics:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`[LAYLASOYOUNG API] Received analytics request for page: ${page}`);
    console.log(`[LAYLASOYOUNG API] Raw referrer: ${referrer}`);
    console.log(`[LAYLASOYOUNG API] Click type: ${click_type || 'page_visit'}`);
    ...
    console.log(`[LAYLASOYOUNG API] Processed referrer: ${readableReferrer}`);

    const analyticsData: BrookeAnalyticsData = {
      page: page || "laylasoyoung",
      referrer: referrer || "",
      readable_referrer: readableReferrer,
      user_agent: userAgent,
      ip_address: ip,
      timestamp: timestamp || new Date().toISOString(),
      pathname: pathname || "/laylasoyoung",
    ...
    if (error) {
      console.error("[LAYLASOYOUNG API] Supabase error:", error);
      return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }

    console.log("[LAYLASOYOUNG API] Successfully saved to Supabase:", data);
    ...
    console.error("[LAYLASOYOUNG API] Error storing analytics:", error);
