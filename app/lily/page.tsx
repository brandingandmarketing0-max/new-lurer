...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const sessionId = `lily_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const sessionKey = `lily_visit_tracked_${sessionId}`;
    ...
    const send = () => {
      try {
        ...
        if (navigator.sendBeacon) {
          ...
          console.log("✅ Lily Analytics - Page visit tracked via sendBeacon");
          
          // Mark this specific session as tracked
          localStorage.setItem(sessionKey, 'true');
          localStorage.setItem('lily_last_tracked', new Date().toISOString());
          setHasTracked(true);
        } else {
          fetch("/api/track", {
            method: "POST",
            ...
            console.log("✅ Lily Analytics - Page visit tracked via fetch");
            ...
            console.error("❌ Lily Analytics - Page visit tracking failed:", error);
          });
        }
      } catch (error) {
        console.error("❌ Failed to track Lily analytics:", error);
      }
    };
  ...
  const handleConfirmAge = () => {
    setShowAgeWarning(false);
    const chars = [104,116,116,112,115,58,47,47,111,110,108,121,102,97,110,115,46,99,111,109,47,112,101,116,105,116,101,108,105,108,115,47,99,49,49,52];
              ...
              <div className="flex flex-col items-center space-y-6">
                ...
                <div className="relative group">
                  ...
                  <Avatar className="relative h-28 w-28 border-4 border-[#B6997B]/20 shadow-lg">
                    <AvatarImage src="https://2eovi9l2gc.ufs.sh/f/XQC8QM7wDFrti55iGMslATwoI4p5NLEYWZtg3UXS2BFR9Gdj" alt="Lily" className="object-cover" />
