...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const sessionId = `rachelll_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const sessionKey = `rachelll_visit_tracked_${sessionId}`;
    ...
    const send = () => {
      try {
        ...
        if (navigator.sendBeacon) {
          ...
          console.log("✅ Rachelll Analytics - Page visit tracked via sendBeacon");
          
          // Mark this specific session as tracked
          localStorage.setItem(sessionKey, 'true');
          localStorage.setItem('rachelll_last_tracked', new Date().toISOString());
          setHasTracked(true);
        } else {
          fetch("/api/track", {
            method: "POST",
            ...
            console.log("✅ Rachelll Analytics - Page visit tracked via fetch");
            ...
            console.error("❌ Rachelll Analytics - Page visit tracking failed:", error);
          });
        }
      } catch (error) {
        console.error("❌ Failed to track Rachelll analytics:", error);
      }
    };
  ...
  const handleConfirmAge = () => {
    setShowAgeWarning(false);
    const chars = [104,116,116,112,115,58,47,47,111,110,108,121,102,97,110,115,46,99,111,109,47,121,111,114,103,105,114,108,114,120,99,104,47,99,54]; // Updated link with new path
              ...
              <div className="flex flex-col items-center space-y-6">
                ...
                <div className="relative group">
                  ...
                  <Avatar className="relative h-28 w-28 border-4 border-[#B6997B]/20 shadow-lg">
                    <AvatarImage src="https://2eovi9l2gc.ufs.sh/f/XQC8QM7wDFrtP9FjMsHIel1suRVCU2AjhJ4rSfb9K65NpqML" alt="Rachelll" className="object-cover" />
          ...
          <div onClick={handleExclusiveContentClick}>
            <Card className="mt-6 relative overflow-hidden border border-[#B6997B]/50 bg-[#B6997B]/10 shadow-lg backdrop-blur-sm cursor-pointer hover:shadow-xl transition-shadow duration-300">
                <CardContent className="p-0">
                  <div className="relative group">
                    <Image
                      src="https://2eovi9l2gc.ufs.sh/f/XQC8QM7wDFrt2f9IgTYudfqRDZPVxm18z9HFCGJtopMw2A3I"
