...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const sessionId = `brooke_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const sessionKey = `brooke_visit_tracked_${sessionId}`;
    ...
    const send = () => {
      try {
        ...
        if (navigator.sendBeacon) {
          ...
          console.log("✅ Brooke Analytics - Page visit tracked via sendBeacon");
          
          // Mark this specific session as tracked
          localStorage.setItem(sessionKey, 'true');
          localStorage.setItem('brooke_last_tracked', new Date().toISOString());
          setHasTracked(true);
        } else {
          fetch("/api/track", {
            method: "POST",
            ...
            console.log("✅ Brooke Analytics - Page visit tracked via fetch");
            ...
            console.error("❌ Brooke Analytics - Page visit tracking failed:", error);
          });
        }
      } catch (error) {
              ...
              <div className="flex flex-col items-center space-y-6">
                ...
                <div className="relative group">
                  ...
                  <Avatar className="relative h-28 w-28 border-4 border-[#B6997B]/20 shadow-lg">
                    <AvatarImage src="https://2eovi9l2gc.ufs.sh/f/XQC8QM7wDFrtKJQTceMoIFQf5TiLHjV7u0D63yYgCwEtxB8Z" alt="Summermae" className="object-cover" />
                ...
                <div className="text-center space-y-2">
                  <h1 className="text-3xl font-bold text-[#8B7355] flex items-center justify-center gap-2">
                    brooke
                    <Sparkles className="h-5 w-5 text-[#B6997B]" />
                  </h1>
                </div>
